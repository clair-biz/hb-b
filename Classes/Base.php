...
//			 echo $q;
		 $res=mysqli_query($obj->con,$q) or die("Error".mysqli_error($obj->con));
//			 print_r($res);
...
//			 echo $q."<br />";
		 $res=mysqli_query($obj->con,$q) or die($obj->con);
//			 print_r($res);
		 if($row=mysqli_fetch_array($res)){
//			 echo $row[0]."<- ->".$row[1];
			 if($row[0]=="Admin" && $row[1]==null && $row[2]==null)
...
public static function prodServList($type){
    $obj=new Base;
    $products=array();
    $services=array();
    if($type=="Product") {
...
//    echo $strQuery4;
    $res=mysqli_query($obj->con,$strQuery4) or die("error ". mysqli_error($obj->con));
        if($row= mysqli_fetch_array($res))
                $id=$row[0];
    return $id;
}
...
//    echo $prodcat1;
    $prod=mysqli_query($obj->con,$prodcat1);
...
//        echo $prodcat;
       $rsprodcat =mysqli_query($obj->con,$prodcat);
        $q=0;
