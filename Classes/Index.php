...
//        $this->con=parent::con;                 // Initiate Database connection
}
...
public function searchServices($search_val){
...
$strprodcat="SELECT distinct cat_id,cat_name,cat_img FROM category where cat_type='Product' and category.cat_id<>0 order by cat_name;";
    ...
    $resprod=mysqli_query($this->con,$strprodcat);
...
$resserv=mysqli_query($this->con,$strservcat);
    $outputserv= mysqli_fetch_all($resserv,MYSQLI_ASSOC);
    return $outputserv;
}

public function subCategoryListByCatName($cat_name){
$str="select cs_name from cat_sub,category where cat_sub.cat_id=category.cat_id and cs_name<>'$cat_name' and cat_name='$cat_name' order by cs_name;";
...
public function getProductsList($prod_name,$city){
...
public function getServicesList($serv_name){
$str="select service.serv_id as 'serv_id', `serv_name`, `serv_img`,serv_desc,cs_name, bname as 'vendor',city_served,vend_rating,vend_rating_off,service.vs_id as 'vs_id',date_format(vend_subscription.na_from as 'na_from','%d/%m/%Y'),date_format(vend_subscription.na_to,'%d/%m/%Y') as 'na_to' from  service,vendor,category,cat_sub,vend_subscription,users WHERE vend_subscription.u_id=users.u_id and vendor.vend_id=users.vend_id and category.cat_id=cat_sub.cat_id and service.cs_id=cat_sub.cs_id and vend_subscription.vs_id=service.vs_id and users.is_active='Y' and service.is_active='Y' and  serv_name like '%".$serv_name."%'";
//echo $str;
$res=mysqli_query($this->con,$str);
    $output= mysqli_fetch_all($res,MYSQLI_ASSOC);
    return $output;
...
public function getCustomerDataForCart($cust_id){
$str="select cust_fname,cust_cntc,cust_addr,cust_email,loc_zip from customer,users where users.cust_id=customer.cust_id and users.cust_id=$cust_id;";  
//echo $str;
$res=mysqli_query($this->con,$str);
    $output= mysqli_fetch_all($res,MYSQLI_ASSOC);
    return $output;
...
$str="select product.prod_id as 'prod_id', `prod_name`,prod_desc,cs_name,cat_name, bname as 'vendor',vend_rating,mrp+(mrp*(tax_table.cgst/100))+(mrp*(tax_table.sgst/100))+ (mrp*(tax_table.cess/100)) as 'mrp',vend_rating_off,prod_unit,prod_qty,product.prod_min_time,mrp_for,mrp_unit,product.vs_id as 'vs_id',vs_pay_status,prod_replace,prod_return,r_within from  product,cat_sub,vendor,category,product_price,vend_subscription,users,tax_table WHERE tax_table.hsn_code=product.hsn_code and vend_subscription.u_id=users.u_id and vendor.vend_id=users.vend_id and category.cat_id=cat_sub.cat_id and cat_sub.cs_id=product.cs_id and product_price.prod_id=product.prod_id and vend_subscription.vs_id=product.vs_id and product.prod_id=".$prod_id.";";
//echo $str;
$res=mysqli_query($this->con,$str);
    $output= mysqli_fetch_all($res,MYSQLI_ASSOC);
    return $output;
...
public function foodSubCategories($city){
//     $type=$obj->type;
