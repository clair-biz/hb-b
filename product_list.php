...
$count=1;
$str="select product.prod_id as 'prod_id', `prod_name`,prod_desc,cs_name,cat_name, bname as 'vendor',vend_rating,vend_rating_off,prod_unit,prod_qty,product.prod_min_time as 'prod_min_time',mrp_for,mrp_unit,product.vs_id as 'vs_id',date_format(vend_subscription.na_from,'%d/%m/%Y') as 'na_from',date_format(vend_subscription.na_to,'%d/%m/%Y') as 'na_to',vs_pay_status,prod_replace,prod_return,r_within,date_format( date_add(now(),interval prod_min_time day), '%Y-%m-%d' ) as 'lead_time',date_format( date_add(now(),interval prod_min_time+1 day), '%d-%m-%Y' ) as 'lead_time_display' from  tax_table,product,cat_sub,vendor,category,product_price,vend_subscription,users WHERE tax_table.hsn_code=product.hsn_code and vend_subscription.u_id=users.u_id and vendor.vend_id=users.vend_id and category.cat_id=cat_sub.cat_id and cat_sub.cs_id=product.cs_id and product_price.prod_id=product.prod_id and vend_subscription.vs_id=product.vs_id and city_served='$city' and users.is_active='Y' and product.is_active='Y' and prod_name like '%".$prod_name."%'";
...
<div class="product-details hide-on-small-and-down0" id="<?php echo "product_".$prod_name;?>" >
        ...
        <div class="product-box col-md-9 col-sm-12" >
            ...
            <div class="row col-md-5 col-sm-12" style="margin-bottom: 0px;">
                <p  class="truncate text-left container-fluid mb-0" style="font-size: 16px;">
	...
	echo "</font>";*/ ?></span-->
          ...
          <p class="text-left container-fluid mb-0">
              <span class="logo-color-g"> <b>Price:</b></span> &#8377;<?php echo Base::getDiscount($prod_details["prod_id"], 1)." Per ".$prod_details["prod_qty"]." ".$prod_details["prod_unit"]; ?>
          </p>
          <?php
    ...
    <input id="<?php echo "reqd_".$prod_details["prod_id"]; ?>" name="reqd" type="text" readonly data-lead="<?php echo $prod_details["lead-time"]; ?>"  data-toggle="datepicker" class="dt form-control" value="<?php echo $prod_details["lead_time_display"]; ?>"   required />
    </div>
...
//    }
    ...
    <p id="<?php echo "subtotal_".$prod_details["prod_id"]; ?>"
       class=" col-sm-12  text-center" style="margin-bottom: 5px;">Subtotal <span class="subtotal badge red white-text"><?php echo "&#8377; ".Base::getDiscount($prod_details["prod_id"], 1)."/-";?></span>
    </p>
    </div>
            </div>
